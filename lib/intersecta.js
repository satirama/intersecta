 ...
 */
const handleEntry = (entry, observer, entryIndex, options) => {
  ...
  } else if (!noOpacityFrames.has(options.animation)) {
...
const createIntersectaObserver = (userOptions) => {
  ...
  const intersectionEffect = (entries, observer) => {
    entries.forEach((entry, index) => handleEntry(entry, observer, index, options));
  };
